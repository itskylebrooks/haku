 ...
 */
export const useAutoScroll = (configOrContainer?: HTMLElement | Window | null | AutoScrollConfig) => {
  // Support both old signature (just container) and new signature (config object)
  const config = configOrContainer && typeof configOrContainer === 'object' && 'scrollContainer' in configOrContainer
  ...
  const updateAutoScroll = useCallback((clientY: number) => {
    ...
    if (!isWindow) {
      ...
      nearBottom = clientY > window.innerHeight - SCROLL_ZONE &&
