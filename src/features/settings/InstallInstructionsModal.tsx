...
export default function InstallInstructionsModal({ open, onClose }: InstallInstructionsModalProps) {
  return (
    <AnimatePresence>
      {open && (
        <motion.div
            ...
            <div className="space-y-4 text-sm text-[var(--color-text-primary)]">
              {/iPad|iPhone|iPod/.test(navigator.userAgent) && !(window as any).MSStream ? (
                ...
                <div className="space-y-2">
                  <p>
                    To install this app, look for the <strong>"Install"</strong> or{' '}
                    <strong>"Add to Home Screen"</strong> option in your browser's menu.
                  </p>
                </div>
              )}
            </div>
